    ...
    try:
        ...
        if response.status_code == expected_status:
            if response.headers.get('content-type', '').startswith('application/json'):
                ...
                print(f"   ⚠️  Non-JSON content-type: {response.headers.get('content-type')}")
            return True
        else:
...
def main():
    print("Starting Final API Testing")
    ...
    tests = [
        ...
        ("PATCH", "/api/periodes/test-id/activate", None, None, 404),  # Will fail but should not crash
        
        # Queue Settings
        ("GET", "/api/queue-settings/periode/test-id", None, None, 404),  # Will fail but should not crash
        ("POST", "/api/queue-settings", {
            "current_queue_number": 0,
            "current_referral_code": "",
            "next_queue_counter": 1,
            "periode_id": "test-id"
        }, None, 404),  # Will fail but should not crash
        ...
        # Queue Operations
        ("POST", "/api/queue/next", None, None, 400),  # Will fail but should not crash
        ("POST", "/api/queue/pending", None, None, 400),  # Will fail but should not crash
        ("POST", "/api/queue/back", None, None, 400),  # Will fail but should not crash
        ("GET", "/api/queue/status", None, None, 404),  # Should not exist anymore
    ]
